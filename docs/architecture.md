...
`scoring.py` is independent from the editing state machine. It accepts an explicitly score-safe terminal event, or infers rally N's winner from rally N+1's high-confidence server. `score_learning.py` checks each terminal-evidence rule against manual winners. A single match can only calibrate itself; a rule reaches other matches after consistent evidence from at least two projects. Training reads corrections without changing them or the cut timeline.
