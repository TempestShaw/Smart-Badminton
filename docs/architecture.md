...
`scoring.py` is independent from the editing state machine. It accepts an explicitly score-safe terminal event, or infers rally N's winner from rally N+1's high-confidence server. Server inference uses owned shuttle proximity to near/far wrists; pose-only candidates stay below the scoring threshold. Manual winner/server corrections are stored separately and no score operation can change a cut.
